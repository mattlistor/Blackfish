...
public class UI {
	...
	Graphics2D g2;
	Font dogica; 
	BufferedImage health_full, health_half, health_empty, playerImage, titleTextImage;
	...
	public int playerLifeAnimationCounter, dialogueBoxAnimationCounter, itemDescriptionBoxAnimationCounter, titleTextAnimationCounter;
	int counter = 0; int counter2 = 50;
	...
	public Entity title_text;
	public int titleTextY, highestBoundTitleTextY, lowestBoundTitleTextY;
	public int titleTextDeltaY = 0; 
	public String titleTextDirection = "down";
	...
	public UI(GamePanel gp) {
		this.gp = gp;
		title_text = new OBJ_Title_Text(gp) ;
		titleTextY = gp.screenHeight / 2 - (gp.tileSize * 3);
		highestBoundTitleTextY = titleTextY;
		lowestBoundTitleTextY = titleTextY+10;
...
//			dradcwMessage();
	...
	private void drawDialogueInstruction() {
		if(gp.player.canStartDialogue == true) {
			g2.setFont(g2.getFont().deriveFont(Font.PLAIN, 10));		
			g2.setColor(Color.white);
			String text = "Press [B] to Talk";
...
//		g2.fillRect(0,  0,  gp.screenWidth,  gp.screenHeight);
		...
		if(counter > 50 && counter < 100 ) {
			counter++;
			counter2--;
			g2.setColor(new Color(0, 0, 0, counter2*5));
			g2.fillRect(0,  0,  gp.screenWidth,  gp.screenHeight);
		}
		if(counter == 100 ) {
			counter = 0;
...
//		}
	...
	public void drawTitleScreen() {
		if(titleScreenState == 0) {
			...
			// BACKGROUND
			g2.drawImage(gp.bg_rocks4, 0, 0, gp.screenWidth,gp.screenHeight, null);
			g2.drawImage(gp.bg_rocks3, 0, 0, gp.screenWidth,gp.screenHeight, null);
			g2.drawImage(gp.bg_rocks2, 0, 0, gp.screenWidth,gp.screenHeight, null);
			g2.drawImage(gp.bg_rocks1, 0, 0, gp.screenWidth,gp.screenHeight, null);
			
			// ENVIRONMENT
			gp.eManager.draw(g2);


			if(titleTextAnimationCounter > 6) {
				// ADJUST THE TITLE TEXT'S Y LEVELS AND DIRECTION;
				if (titleTextDirection == "up") {

					if(titleTextY < highestBoundTitleTextY ) {
						titleTextDirection = "down";
					}
				}
				else if (titleTextDirection == "down") {
					if(titleTextY > lowestBoundTitleTextY ) {
						titleTextDirection = "up";

					}
				}
				switch(titleTextDirection) {
					case("down"): titleTextY+=1; break;
					case("up"): titleTextY-=1; break;
				}
				titleTextAnimationCounter = 0;
			}	
			titleTextAnimationCounter++;
			
			
			// TITLE TEXT WITH SHINE
			int y = gp.tileSize*5-32;
			int x = gp.screenWidth/2 - (gp.tileSize*2)/2 ;
			y += gp.tileSize*1.5;
			title_text.spriteNumAndMovementHandler();
			switch(title_text.spriteNum){
			case(1):titleTextImage = title_text.frame1;break;
			case(2):titleTextImage = title_text.frame2;break;
			case(3):titleTextImage = title_text.frame3;break;
			case(4):titleTextImage = title_text.frame4;break;
			case(5):titleTextImage = title_text.frame5;break;
			case(6):titleTextImage = title_text.frame6;break;
			case(7):titleTextImage = title_text.frame7;break;
			case(8):titleTextImage = title_text.frame8;break;

			}

			
			g2.drawImage(titleTextImage, gp.screenWidth / 2 - (gp.tileSize * 4), titleTextY, null);
			...
			if (spriteNum == 1) {
				playerImage = gp.player.right1;
			}
			if (spriteNum == 2) {
				playerImage = gp.player.right2;
			}
			if (spriteNum == 3) {
				playerImage = gp.player.right3;
			}
			if (spriteNum == 4) {
				playerImage = gp.player.right4;
			}
			g2.drawImage(playerImage, gp.screenWidth / 2 - (gp.tileSize / 2), gp.screenHeight / 2 - (gp.tileSize / 2), gp.tileSize, gp.tileSize, null);
			spriteCounter++;
			
			// MENU
			g2.setColor(new Color(132, 100, 0));
			g2.setFont(g2.getFont().deriveFont(Font.PLAIN, 20));
			text = "PLAY";
			x = getXforCenteredText(text)-gp.tileSize;
			y += gp.tileSize*1;
...
//			drawDialogueWindow(x, dialogueBoxY-gp.tileSize, width,  40);
		...
		if(npc.dialogues[npc.dialogueSet][npc.dialogueIndex] != null) {
			...
			if(gp.gameState == gp.dialogueState) {
				npc.dialogueSet++;
				if(npc.dialogues[npc.dialogueSet][0] == null) {
					// option 1: npc resets to first set of dialogue
					npc.dialogueSet = 0;
					// option 2:  npc repeats last set of dialogue
//					dialogueSet--;
				}
				System.out.println("here! -- "+npc.dialogues[0][0]+npc.name);
				gp.gameState = gp.playState;
			}
