...
public class MON_Crab extends Entity {
	...
	public void spriteNumAndMovementHandler() {
		...
		if(collisionOn == false ) {
			switch (direction) {
				case "up": {worldY -= speed; worldX += speed;} break;
				case "down": {worldY += speed;  worldX -= speed;}break;
				case "left": {worldX -= speed;  worldY -= speed;}break;
				case "right": {worldX += speed;  worldY += speed;}break;
			}
		}
		else if(collisionOn == true) {
//			Random random = new Random();
//			int i = random.nextInt(2)+1; // number from 1-2			
